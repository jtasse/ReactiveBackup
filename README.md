...
| `excludedRepoSubfolders`             | Array of subfolders to exclude from the backup. (Ignored if `includedRepoSubfolders` is populated). Defaults include `.git`, `node_modules`, `dist`, `.next`, `.venv`, `__pycache__`, `.pytest_cache`, and `.aws-sam`. |
